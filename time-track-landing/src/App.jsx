...
function Logo({ inverse = false }) { return <a className={`logo ${inverse ? 'logo--inverse' : ''}`} href="#top" aria-label="TrackOn home"><img src={inverse ? '/assets/logo-footer.svg' : '/assets/logo-header.svg'} alt="TrackOn" /></a>; }
...
function FeatureCard({ title, text, image, tone, titleTone }) { return <article className={`feature-card feature-card--${tone}`}><div><h3 className={`feature-card__title feature-card__title--${titleTone}`}><span>{title[0]}</span><span>{title[1]}</span></h3><p>{text}</p></div><img src={`/assets/${image}`} alt="" /></article>; }
